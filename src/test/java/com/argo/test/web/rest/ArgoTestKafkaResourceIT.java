...
class ArgoTestKafkaResourceIT {
    ...
    void setup() {
        ...
        ArgoTestKafkaResource kafkaResource = new ArgoTestKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/argo-test-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/argo-test-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
